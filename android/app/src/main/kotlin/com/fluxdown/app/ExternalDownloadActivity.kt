...
class ExternalDownloadActivity : FlutterActivity() {
        ...
        return when (intent.action) {
            ...
            Intent.ACTION_VIEW -> {
                ...
                if (data.startsWith("fluxdown://", ignoreCase = true)) {
                    ...
                    // 三个 extra。UA/Referer 并入请求头 JSON，Cookie 单体承载，
                    // 与 Dart 侧 headers/cookies 字段对应，弹层高级选项预填。
                    sharePayload(
                        data,
                        cookies = intent.getStringExtra("Cookie").orEmpty(),
                        headers = buildXBrowserHeaders(
    ...
    /** 把 X 浏览器附带的 UA / Referer 组装为请求头 JSON 串（均在则都写入）。 */
    private fun buildXBrowserHeaders(userAgent: String?, referer: String?): String {
        if (userAgent.isNullOrEmpty() && referer.isNullOrEmpty()) return ""
        return try {
            org.json.JSONObject().apply {
                if (!userAgent.isNullOrEmpty()) put("User-Agent", userAgent)
