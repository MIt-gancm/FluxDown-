...
- `auto_proxy.rs`：`ProxyMode::Auto` 决策机器——任务直连启动（保留 CDN 聚合资格），慢任务经候选代理（系统代理，回退手动字段）采 256KB 样，单连接吞吐 ≥2× 才经 `NodePool::switch_to_client` 分段边界热切换；host 级决策缓存两层：内存租约（`DecisionCache`，重启清零）+ `route_health.rs` 持久化先验（config `auto_route_health`，网络指纹 epoch——换网整表丢弃、**离线=unknown 不清表**（暂存待上线采纳）；Cooldown 指数退避 300s×2^n（计数 24h 保活跨窗升级）/ NoSwitch 24h（且跨重启挡 forward failover）；Proxy 胜绩按 confirm_ts 计分时钟记「不同天」确认——单日仅缩短采样等待期 10s→6s，≥2 天且 72h 内有实证胜出才代理起飞（复用 `proxy:cached` 标签；**带局部数据的 resume 降档**防 If-Range 失配清进度），有效期 24h×2^(n-1) 封顶 7 天，完成即续期）；failover 双向且**独立于重试配额**：直连连接类失败 → 重试转代理；代理路由连接类失败且本机有默认路由 → 作废先验+内存冷却回直连（自愈，杜绝锁死）；采样 validator 不一致 = 禁切换（完整性优先）；胜绩只在切换实际生效后落盘。任务级最终链路落 `tasks.auto_route` + `EngineEvent::TaskRouteChanged`（wire 标签见 `auto_proxy::route`），双端详情面板「链路」行可追溯。代理设置变更（`set_proxy_config`）同点清 `DecisionCache` + `route_health::clear_all` + `domain_conn_caps`。
...
- **env**（`config.rs`）：`FLUXDOWN_DATA_DIR`、`FLUXDOWN_DATABASE_URL`（`sqlite:`/`postgres:`）、`FLUXDOWN_BIND`（默认 `0.0.0.0:17800`——**注意非回环**，与桌面不同）、`FLUXDOWN_WEBROOT`（SPA 目录，默认 exe 同级 `./web`）、`FLUXDOWN_TOKEN`（预置访问密钥，仅库中无密钥时采纳）、`FLUXDOWN_DEMO`/`FLUXDOWN_DEMO_URL`、`FLUXDOWN_LANG`、`FLUXDOWN_ANALYTICS`(off 硬关)/`FLUXDOWN_ANALYTICS_APP_KEY`。`ensure_server_config`：强制 `local_server_api_enabled=true`、seed mcp、**不再自动生成 token**——库中无密钥即返回空串进入「待设置」态（管理 API 全线 403，stderr 打引导横幅），由 Web 首次运行向导落定。
...
- `routes_ext.rs`：管理 token 保护（config get/put、queues CRUD+启停/定时/排序、task 移队+boost、fs_list、proxy_test、token/regenerate、stats、logs、bt tracker-sub refresh、**component ffmpeg/ytdlp status/versions/install/uninstall**）；开放（`?token=` query，浏览器不能设头）：`GET /api/v1/ws`、`tasks/{id}/file` 流式取回、logs/export、openapi.json、Scalar `/docs`；**完全无鉴权**：`GET /api/v1/setup/status` + `POST /api/v1/setup`（首次运行向导，仅在密钥未设定时可写，设过即 409）。
