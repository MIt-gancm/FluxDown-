...
mod inner {
    ...
    pub fn is_associated() -> bool {
        ...
        // SAFETY: `uti.raw()` is a valid CFStringRef; the returned handler ref
        // is owned by us and released via the `CfOwned` guard below.
    ...
    pub fn associate() -> Result<(), io::Error> {
        ...
        // SAFETY: both CFStringRefs are valid and alive for the duration of the
        // call; the function does not take ownership of them.
        let status =
    ...
    pub fn disassociate() -> Result<(), io::Error> {
        ...
        let uti = cf_string(TORRENT_UTI)?;
        let empty = cf_string("")?;
        // SAFETY: both CFStringRefs are valid and alive for the call.
        let status =
