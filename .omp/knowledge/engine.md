...
- `auto_proxy.rs`：`ProxyMode::Auto` 决策机器——任务直连无阻塞启动（保留 CDN 聚合资格），越过 6s 爬升期且剩余 ≥4MiB 时经候选代理（系统代理，回退手动字段）采 256KiB 样；按相同的单连接量纲比较（不让多分段总速掩盖慢连接），代理吞吐 ≥2× 才经 `NodePool::switch_to_client` 在分段边界热切换。host 级决策缓存两层：内存租约（`DecisionCache`，重启清零）+ `route_health.rs` 持久化先验（config `auto_route_health`，网络指纹 epoch——换网整表丢弃、**离线=unknown 不清表**）；Cooldown 指数退避 / NoSwitch 完整性门禁；Proxy 胜绩按不同天确认，单日缩短复评等待，≥2 天且 72h 内有实证胜出才代理起飞。局部续传永不直接采纳 host 代理租约，必须重新采样 validator。failover 双向且每轮一次、**独立于通用重试配额**：直连建连或传输中断 → 立即换代理，代理中断且本机有默认路由 → 作废先验并立即回直连；两条链都坏时停止换路，后续只服从通用重试，杜绝 ping-pong。任务级最终链路落 `tasks.auto_route` + `EngineEvent::TaskRouteChanged`（含 `direct:failover` / `proxy:failover`），双端详情面板可追溯。代理设置变更同点清 `DecisionCache` + `route_health` + `domain_conn_caps` + failover 状态。
