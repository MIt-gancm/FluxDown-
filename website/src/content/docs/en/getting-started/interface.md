...
- **Queues** (expanded by default) — the built-in Default Queue plus any named queues you've created. Click **+** to create a new queue (name, speed limit, max concurrent, save directory, default threads, default User-Agent). Hover or right-click a named queue to edit or delete it.
